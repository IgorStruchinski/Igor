...
public partial class Cameras : System.Web.UI.Page
{
    ...
    {     
        ...
        {
            Response.Redirect("http://localhost:58277/AccessDenied.aspx");
        }
        else
        {
    ...
    protected void MainSVNProc(GridView GV, string CamsFileName)
    {
        ...
        for (int i = 0; i < Cams.Length; i++)
        {
            ...
            f = new StreamReader("D:\\1\\CAM_" + tag, System.Text.Encoding.Default);
            cam_date = f.ReadLine();
            cam_time = f.ReadLine();
            cam_ip = f.ReadLine();
            cam_name = f.ReadLine();
        ...
        for (int r = 0; r < GV.Rows.Count; r++)
        {
            ...
            GV.Rows[r].Cells[2].Visible = false; //закоментировать чтобы вывести ip адреса
        }
        GV.HeaderRow.Cells[5].Visible = false;
        GV.HeaderRow.Cells[2].Visible = false;//закоментировать чтобы вывести ip адреса
